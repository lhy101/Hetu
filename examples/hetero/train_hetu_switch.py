...
from hetu.utils.data import HetuJsonDataset, build_data_loader
from hetu.utils.parallel import distributed_init, read_strategy, read_ds_parallel_config, parse_multi_ds_parallel_config
...
def pretrain(args):
    ...
    def run_plan(
        ...
        # 异构策略
            ...
            unused_rank_list = args.unused_rank
            ...
            if args.hetero_layers == []:
                ...
                hetero_stages = [len(pipeline) for pipeline in args.hetero_layers]
            ...
            if args.micro_batch_num_list == []:
                ...
                micro_batch_num_list = args.micro_batch_num_list
    ...
    # 热切换
    def test_switch(from_strategy_id, to_strategy_id): 
        strategy_paths = args.strategy_config.split(',')
        assert len(strategy_paths) == args.num_strategy, \
            f'strategy_config num should equal to num_strategy {args.num_strategy}'
        strategy_configs = []
        for strategy_path in strategy_paths:
            strategy_configs.append(read_strategy(strategy_path))
        args.hetero = True
        args.torch_profile = False
        consumed_samples = 0 # should be reset when run next epoch
        cp_list = strategy_configs[from_strategy_id]["cp_list"]
        args.gpus_per_stage = strategy_configs[from_strategy_id]["tp"]
        args.hetero_layers = strategy_configs[from_strategy_id]["layers_num_list"]
        args.micro_batch_num_list = strategy_configs[from_strategy_id]["micro_batch_num_list"]
        args.rank_to_device_mapping = strategy_configs[from_strategy_id]["rank_to_device_mapping"]
        args.unused_rank = strategy_configs[from_strategy_id]["unused_rank"]
        args.seq_len_list = strategy_configs[from_strategy_id]["seq_len_list"]
        consumed_samples = run_plan(cp_list,
                                    epoch = 0,
                                    steps = 1,
                                    ...
                                    run_level = ht.run_level("update"))
        cp_list = strategy_configs[to_strategy_id]["cp_list"]
        args.gpus_per_stage = strategy_configs[to_strategy_id]["tp"]
        args.hetero_layers = strategy_configs[to_strategy_id]["layers_num_list"]
        args.micro_batch_num_list = strategy_configs[to_strategy_id]["micro_batch_num_list"]
        args.rank_to_device_mapping = strategy_configs[to_strategy_id]["rank_to_device_mapping"]
        args.unused_rank = strategy_configs[to_strategy_id]["unused_rank"]
        args.seq_len_list = strategy_configs[to_strategy_id]["seq_len_list"]
        consumed_samples = run_plan(cp_list,
                                    epoch = 0,
                                    steps = 1,
                                    consumed_samples = consumed_samples, 
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--ds_parallel_config", type=str, help="ds parallel config json file"
    )
    parser.add_argument(
        "--strategy_config", type=str, help="ds parallel config json file"
    )
    parser.add_argument(
